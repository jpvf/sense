...
class Validation {
	...
	public static function getInstance()
	{
        if ( ! self::$instance) 
        { 
            self::$instance = new form_validation(); 
        } 
        return self::$instance; 
	}


	function set_rules($field = NULL, $label = '', $rules = '')
	{
		// No reason to set rules if we have no POST data or No fields? Nothing to do..
		if (empty($_POST) OR is_null($field))
		{
			return;
		}
...
/**
    ...
    {
        $pattern = "/^(http(s?)\:\/\/)?([w]{2}([\w\-]+\.)+([\w]{2,5}))(:[\d]{1,5})?/";
    ...
    {
    	if ( ! preg_match( '/^[0-9]+$/', $str) OR $str == 0)
    	{
    		return FALSE;
    	}
	...
	{
		return Input::getInstance()->strip_image_tags($str);
	}
	...
	{
		return Input::getInstance()->xss_clean($str);
	}
